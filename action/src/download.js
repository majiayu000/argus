...
function requestBytes(url, limit, redirects = 0, accept = API_ACCEPT, expectedContentType = null, githubToken) {
  ...
  if (redirects === 0) headers.Authorization = `Bearer ${githubToken}`;
  return new Promise((resolve, reject) => {
    const request = https.get(parsed, { headers, timeout: 30_000 }, (response) => {
      if ([301, 302, 303, 307, 308].includes(response.statusCode)) {
        ...
        return requestBytes(new URL(response.headers.location, parsed).toString(), limit, 1, accept, expectedContentType, githubToken).then(resolve, reject);
      }
...
async function fetchRelease(version, githubToken) {
  const bytes = await requestBytes(`https://api.github.com/repos/majiayu000/argus/releases/tags/v${version}`, 1024 * 1024, 0, API_ACCEPT, "application/json", githubToken);
...
async function fetchTagCommit(version, githubToken) {
  const refBytes = await requestBytes(`https://api.github.com/repos/majiayu000/argus/git/ref/tags/v${version}`, 1024 * 1024, 0, API_ACCEPT, "application/json", githubToken);
  ...
  const tagBytes = await requestBytes(`https://api.github.com/repos/majiayu000/argus/git/tags/${ref.object.sha}`, 1024 * 1024, 0, API_ACCEPT, "application/json", githubToken);
...
async function downloadVerifiedAsset(release, name, limit, githubToken) {
  const asset = uniqueAsset(release, name);
  if (asset.size > limit) throw new Error(`release asset exceeds size limit: ${name}`);
  const bytes = await requestBytes(asset.url, limit, 0, ASSET_ACCEPT, null, githubToken);
  if (bytes.length !== asset.size || `sha256:${sha256(bytes)}` !== asset.digest) throw new Error(`GitHub asset digest mismatch: ${name}`);
  return bytes;
}

async function verifyAttestation(subject, bundle, version, commit, runner = runBounded, githubToken) {
  const commandEnv = { ...process.env, GH_TOKEN: githubToken };
  const help = await runner("gh", ["attestation", "verify", "--help"], { timeoutMs: 15_000, stdoutLimit: 1024 * 1024, env: commandEnv });
  ...
  const result = await runner("gh", ["attestation", "verify", subject, "--bundle", bundle, "--repo", "majiayu000/argus", "--cert-oidc-issuer", "https://token.actions.githubusercontent.com", "--signer-workflow", workflow, "--source-ref", ref, "--source-digest", commit, "--deny-self-hosted-runners", "--format", "json"], { timeoutMs: 180_000, stdoutLimit: 4 * 1024 * 1024, env: commandEnv });
...
async function materializeRelease(version, target, tempDir, validateManifest, dependencies = {}) {
  ...
  const runner = dependencies.runBounded || runBounded;
  const githubToken = dependencies.githubToken;
  const [release, commit] = await Promise.all([fetchReleaseFn(version, githubToken), fetchTagCommitFn(version, githubToken)]);
  const manifestBytes = await downloadFn(release, "release_manifest.json", 1024 * 1024, githubToken);
  const manifestBundle = await downloadFn(release, "release_manifest.sigstore.json", 4 * 1024 * 1024, githubToken);
  ...
  await verifyFn(manifestPath, manifestBundlePath, version, commit, runner, githubToken);
  ...
  const binary = await downloadFn(release, binaryName, 128 * 1024 * 1024, githubToken);
  ...
  const bundle = await downloadFn(release, bundleName, 4 * 1024 * 1024, githubToken);
  ...
  await verifyFn(binaryPath, bundlePath, version, commit, runner, githubToken);
  return binaryPath;
}
