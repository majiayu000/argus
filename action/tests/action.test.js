...
const config = { schemaVersion: 1, defaultBinaryVersion: "0.2.0", compatibilityRange: ">=0.2.0,<0.3.0" };
...
test("input contract rejects empty and open-ended values", () => {
  ...
  assert.throws(() => readInputs({ INPUT_SCANTYPE: "package", INPUT_PATH: ".", INPUT_ARGUSVERSION: "0.1.0" }, config), /outside/);
  assert.deepEqual(readInputs({ INPUT_SCANTYPE: "agent", INPUT_PATH: "." }, config), { scanType: "agent", inputPath: ".", format: "text", version: "0.2.0", failOn: "block", base: "", baseLockfileFormat: "", maliciousDb: "", approvalLedger: "" });
...
test("manifest validates identity and complete asset matrix", () => {
  ...
  const assets = Object.entries(runners).flatMap(([target, runner]) => ["binary", "archive"].map((kind) => ({ name: kind === "binary" ? `argus-v0.2.0-${target}${target.endsWith("windows-msvc") ? ".exe" : ""}` : `argus-v0.2.0-${target}.${target.endsWith("windows-msvc") ? "zip" : "tar.gz"}`, target, runner, kind, size: 1, sha256: "b".repeat(64) })));
  ...
  const value = { schemaVersion: 1, binaryVersion: "0.2.0", tag: "v0.2.0", commit: "a".repeat(40), assets };
  assert.equal(validateManifest(value, "0.2.0", "a".repeat(40)), value);
  assert.throws(() => validateManifest({ ...value, extra: true }, "0.2.0", "a".repeat(40)), /keys/);
  assert.throws(() => validateManifest({ ...value, assets: assets.slice(1) }, "0.2.0", "a".repeat(40)), /incomplete/);
});

test("reports bind decision to exit code and format", () => {
  assert.equal(validateReport("text", "decision: allow  package: x\npath: .\nfindings: none\n", 0, "0.2.0", "package"), "allow");
  const blocked = JSON.stringify({ artifact: "package-dir", path: ".", package_name: "x", package_version: "1.0.0", decision: "block", findings: [{}] });
  assert.equal(validateReport("json", blocked, 1, "0.2.0", "package"), "block");
  assert.equal(validateReport("json", JSON.stringify({ ...JSON.parse(blocked), vulnerability: { version: 1 }, future_additive_field: true }), 1, "0.2.0", "package"), "block");
  assert.throws(() => validateReport("json", JSON.stringify({ ...JSON.parse(blocked), vulnerability: [] }), 1, "0.2.0", "package"), /contract/);
  ...
  assert.equal(validateReport("json", lockfile, 1, "0.2.0", "lockfile"), "block");
  const sarif = JSON.stringify({ version: "2.1.0", runs: [{ tool: { driver: { name: "argus", version: "0.2.0" } }, invocations: [{ executionSuccessful: true }], results: [] }] });
  assert.equal(validateReport("sarif", sarif, 0, "0.2.0", "agent"), "allow");
  assert.equal(validateReport("sarif", JSON.stringify({ version: "2.1.0", runs: [] }), 1, "0.2.0", "lockfile"), "block");
  assert.throws(() => validateReport("json", JSON.stringify({ ...JSON.parse(blocked), decision: "allow" }), 1, "0.2.0", "package"), /contract/);
...
test("main preserves blank outputs on early failure and only exitCode on invalid report", async () => {
  ...
  assert.deepEqual(values, { decision: "", exitCode: "", reportPath: "", sarifFile: "", argusVersion: "0.2.0" });
  fs.writeFileSync(output, "");
  let calls = 0;
  await main(env, { targetFor: () => "x86_64-unknown-linux-gnu", materializeRelease: async () => "/tmp/argus", runBounded: async () => (++calls === 1 ? { code: 0, stdout: "argus 0.2.0\n", stderr: "" } : { code: 1, stdout: "{}", stderr: "" }) });
  values = Object.fromEntries(fs.readFileSync(output, "utf8").trimEnd().split("\n").map((line) => line.split("=", 2)));
  assert.deepEqual(values, { decision: "", exitCode: "1", reportPath: "", sarifFile: "", argusVersion: "0.2.0" });
