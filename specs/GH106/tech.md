...
2. 将路径和完整 discovery 得到的 `skill_dirs` 交给唯一 membership API
   `surface::classify(&root_context, logical_path, skill_dirs)`，把结果写入
   `surface_kind`。snapshot 模块不出现文件名、扩展名、root-name、prefix 或
   目录形状常量。
...
2. 先完成不排除 snapshot target 的 complete discovery 与 `skill_dirs` 计算。
   ...
   relative path 和 skill dirs 调用同一个 root-aware `surface::classify`；即使
   update target 尚不存在也按其计划路径分类。
...
唯一 membership API 改为 root-aware：

```text
ScanRootContext::from_canonical_scan_root(root_path, root_entry_type)
surface::classify(&root_context, logical_path, logical_skill_dirs)
...
`ScanRootContext` 的 prefix 规则冻结如下：
...
- 其他 root 不加 prefix；`AGENTS.md`、`.cursorrules` 等 basename shape 仍按
  ...
  discovery 得到的 `logical_skill_dirs`，并由同一 context 同步 qualify；不得
  向 PATH 外探测 `SKILL.md`。
...
规则。legacy collector、complete inventory、semantic projection 和
inside-root snapshot target（包括尚不存在的 update target）都必须把同一个
`ScanRootContext`、root-relative logical path 与 skill dirs 传给这一个 API。
...
- `crates/argus-agent/src/surface.rs`：唯一 root-aware membership API、
  `ScanRootContext` 与 inventory-only kind。
- `crates/argus-agent/src/lib.rs`：SnapshotMode 分流、legacy collector 的
  InventoryOnly no-op、共享 root context、complete `DiscoveredEntry`
...
| B-010 | root-aware classifier、mode split 与正交 projection | `cargo test -p argus-agent --test gh106_snapshot root_context_classification_matrix` |

## 数据流

共同前缀只有 `PATH + SnapshotMode + BaselineMode` → strict identity/path
validation → `surface::ScanRootContext`，随后按 mode 分流：

```text
SnapshotMode::None (including AGT-02-only)
  → legacy .git/node_modules-pruned collector
  → root-aware classify(logical_path, logical_skill_dirs)
...
SnapshotMode::Check | Update
  ...
  → root-aware classify shared by entries and snapshot target guard
...
- [ ] Root context: `~/.claude/settings.json` directory/single-file、
