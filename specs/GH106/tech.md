...
inventory walker 沿用 root symlink 拒绝与 `follow_links(false)`，但不得使用
`filter_entry` 或等价逻辑在分类前按 `.git`、`node_modules` 或其他 ancestor
basename 剪掉 subtree。metadata discovery 必须遍历每个非 symlink directory，
因为未分类 ancestor 的后代仍可能是 `AGENTS.md` 或 `.claude/**`。对 discovery
看到的每个 entry：
...
2. 先完成不排除 snapshot target 的 metadata discovery 与 `skill_dirs` 计算。
...
metadata discovery 对 `.git`/`node_modules` 也必须下钻，分类完成后才丢弃
`None` 或在 semantic mode 跳过 `InventoryOnly`。因此
...
inventory fail closed，不能以“原先会 prune”为由忽略。
...
1. 规范化 root/snapshot identity；执行不按 ancestor basename 剪枝的
   metadata discovery，计算 `skill_dirs`，并对 root 内 snapshot target 执行
   canonical membership guard。classified target 立即 operational reject。
...
| B-010 | non-pruning discovery、`surface::classify`/InventoryOnly 与 semantic collector | `cargo test -p argus-agent --test gh106_snapshot protected_descendants_reach_classifier` |

## 数据流

共同前缀为 `PATH + SnapshotMode + BaselineMode` → strict identity/path
validation → non-pruning metadata discovery + `skill_dirs` →
root-contained snapshot target canonical membership guard → permitted exact
self-exclusion → non-following full inventory hash → optional v1 snapshot
load/compare → sorted AGT-04 findings → existing semantic rules/AGT-02/judge →
report/decision ready in memory。classified snapshot target 在 guard 处直接
operational failure，不进入 exclusion/load/render/write。此后只能进入以下
互斥分支：
...
- 在 classification 前继续 prune `.git`/`node_modules`：拒绝，会遗漏受支持
  high-context descendant；必须先遍历/分类，再丢弃 `None` 或跳过
  `InventoryOnly`。
...
- Performance: file hash 读取全部字节且无语义 size cap；固定 buffer 保持 O(1)
  额外内存，walker 和 compare 为 O(n log n)。为发现 protected descendants，
  metadata discovery 不再 prune `.git`/`node_modules`；只 hash classified
  entries，但大型 tree 的 metadata walk 成本必须由 benchmark/文档如实说明，
  不能用 silent prune 换取性能。
...
- [ ] Descendant completeness: `.claude/node_modules/**`、`.claude/.git/**`、
      `node_modules/pkg/AGENTS.md` 与 `.git/**/.cursorrules` 到达 classifier 并进入
      inventory；普通 unclassified descendant 不进入；深层 walk error fail closed。
