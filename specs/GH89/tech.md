...
共享 decision derivation。`crates/argus-rules/src/decision.rs` 新增闭集
`APPROVAL_ONLY_RULES = {version-shape-anomaly, rapid-publish-window}`，并把两个
`*-unassessed` rule 加入 `INFO_ONLY_RULES`。派生时先移除纯 Info rules，再将
approval-only rules 从残余集合分离：残余为空时返回 approval；残余按现有逻辑为
approval 时仍返回 approval；残余为 block 时必须返回 block。不得在 `argus-fetch`
调用点覆盖 decision，也不得把任意 Medium finding 泛化为 approval。

## Planned Changes Manifest

<!-- specrail-planned-changes
{"version":1,"issue":89,"complete":true,"paths":["Cargo.lock","Cargo.toml","README.md","crates/argus-cli/src/main.rs","crates/argus-cli/tests/npm_anomaly_cli.rs","crates/argus-fetch/Cargo.toml","crates/argus-fetch/src/anomaly.rs","crates/argus-fetch/src/lib.rs","crates/argus-fetch/src/packument.rs","crates/argus-fetch/tests/anomaly.rs","crates/argus-rules/src/decision.rs","docs/supply-chain-attacks.md","specs/GH89/product.md","specs/GH89/tasks.md","specs/GH89/tech.md"],"spec_refs":["specs/GH89/product.md","specs/GH89/tech.md","specs/GH89/tasks.md"]}
-->
...
| B-007 | shared approval-only/info-only rule sets + residual decision merge | `cargo test -p argus-rules anomaly_decision` |
...
unassessed findings，与 package/provenance findings 合并，再由扩展后的共享决策
函数按 residual-block 优先级派生并交给 renderer 输出。任何必需元数据错误在
report 输出前向上传播。
