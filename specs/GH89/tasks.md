...
- [ ] `SP89-T2` 在同一 registry base 上实现一页 npm search 候选读取、精确 `_npmUser.name`/`publisher.username` 关联、24 小时/5 包检测，以及 250 对象、2 MiB、同源/base-path、15 分钟缓存和闭集错误语义。Covers: B-005, B-006, B-008, B-009. Owner: implementation worker. Dependencies: SP89-T1. Done when: mock transport 的正负、重复/乱序、候选不足 unassessed、截断/schema/cache/redirect/transport 失败矩阵全部通过；cache key 区分完整 registry base URL 与目标发布时间，且 `fetched_at < target_published_at` 必须重新抓取；单次 scan 每个 publisher 不超过一次请求。Verify: `cargo test -p argus-fetch rapid_publish_window`; `cargo test -p argus-fetch rapid_publish_benign`; `cargo test -p argus-fetch anomaly_transport`.
