...
增加结构化谓词区分两类事实：通用 execution fact，以及首个字符串参数可由
现有 `remote_shell_command_string` 检查的 string executor。exec wrapper、
`eval` 与 shell `iex` 进入同一有界路径；`function` 等不执行首参 shell
字符串的构造器不自动纳入。解析仍只进行一次，不递归处理嵌套求值。
...
| --- | --- | --- |
| B-001 | `classify.rs` string-executor predicate | `cargo test -p argus-agent gh102_eval` |
...
| B-006 | `syntax/reference.rs` mixed assignment provenance | `cargo test -p argus-agent gh102_assignment_provenance` |
...
- Maintenance: string executor 与端点形状必须集中定义，禁止分散 callee 特判。
