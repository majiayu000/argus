...
- [ ] `SP102-T3` 在 syntax/static-value 边界保留“可执行引用 + 相邻字面片段”的有序 assignment provenance，并在 capability 聚合层区分可参与网络相关性的读取与仅需保留 manifest 的本地引用；纯字面与动态片段不得被猜测。Covers: B-006, B-007, B-008, B-010. Owner: implementation worker. Dependencies: none. Done when: shell 混合路径来源被后续网络 fact 还原，assignment-only credential-access 仍可见，纯字面以及“本地使用 + 无关联网”负例保持非阻断。Verify: `cargo test -p argus-agent gh102_assignment_provenance`.
