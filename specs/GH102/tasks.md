...
- [ ] `SP102-T1` 统一 exec wrapper、`eval` 与 shell `iex` 的首参字符串执行形状，并复用一次有界管道解析。Covers: B-001, B-002, B-003, B-008, B-010. Owner: implementation worker. Dependencies: none. Done when: 求值器正例产生现有 remote-execution finding，动态/良性/嵌套负例不升级。Verify: `cargo test -p argus-agent gh102_eval`.
...
- SP102-T1 仅拥有 `crates/argus-agent/src/capability/classify.rs` 中的
  string-executor 区域及对应 evaluator tests。
