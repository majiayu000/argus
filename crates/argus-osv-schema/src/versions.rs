//! Per-ecosystem version parsing and total ordering used by OSV range
//! matching. Extracted from `argus-intel`'s matcher (#135) so that both the
//! offline malicious-package matcher and the online OSV client share one
//! comparison semantics per ecosystem.
...
pub fn parse_version(ecosystem: Ecosystem, raw: &str) -> Result<()> {
...
pub fn compare_versions(ecosystem: Ecosystem, left: &str, right: &str) -> Result<Ordering> {
    Ok(match ecosystem {
