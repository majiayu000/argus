...
| --- | --- | --- | --- | --- |
| `rclean` | `smoke` | `needs_human` | `examples/rclean-smoke.md`, external `majiayu000/specrail:specs/GH5/*`, `evaluate.py`, `tests/test_evaluate.py` | Promote from read-only smoke to an explicit target-repo integration plan before writing into `rclean`. |
| `litellm-rs` | `pr_gate` | `active` | External `majiayu000/specrail:specs/GH7/*`, `tests/test_pr_gate.py`, PR `majiayu000/litellm-rs#718` | Add reusable PR evidence fixtures for more review-thread and CI states. |
...
checks that the three known pilots stay present, SpecRail-local evidence paths
exist, and external evidence records include an explicit repository.
