...
| `AGT-06-workflow-context-injection` | critical → block | a workflow or local composite Action interpolates attacker-controlled GitHub event data directly into an inline `run` script or first-party `actions/github-script` `with.script` (including via `${{ env.* }}` re-interpolation of a previously tainted env assignment); safe remediation passes data through `$ENV` / `process.env` without `${{ }}` in the script |
