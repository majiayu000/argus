...
- **M2 plumbing** — the DSSE, Fulcio-chain, Rekor-inclusion, and OIDC identity-policy path is opt-in behind the `sigstore` Cargo feature ([#14](https://github.com/majiayu000/argus/issues/14)). The current upstream verifier cannot fully verify real npm v0.2 `intoto/0.0.2` bundles, so even otherwise legitimate bundles remain fail-closed as Critical `provenance-signature-invalid` with a `Block` decision rather than being downgraded or reported Verified. Builds without the `sigstore` feature hard-error when `--verify-sigstore` is requested (also fail closed). End-to-end support in [#128](https://github.com/majiayu000/argus/issues/128) is not yet complete; see [`docs/design/sigstore-verification.md`](docs/design/sigstore-verification.md) §10.
