...
- **M2 plumbing** — the DSSE, Fulcio-chain, Rekor-inclusion, and OIDC identity-policy path is opt-in behind the `sigstore` Cargo feature ([#14](https://github.com/majiayu000/argus/issues/14)). The current upstream verifier cannot handle real npm v0.2 `intoto/0.0.2` bundles, so they surface as Info `provenance-signature-unverified` (Unsupported — a capability gap, not a signature failure) rather than a green Verified verdict; genuine signature failures still produce Critical `provenance-signature-invalid`. Builds without the `sigstore` feature hard-error when `--verify-sigstore` is requested (fail closed). See [`docs/design/sigstore-verification.md`](docs/design/sigstore-verification.md) §10.
