...
use crate::verify_impl::helpers::{
    compute_artifact_digest_algo, verify_dsse_artifact_binding, verify_message_signature_crypto,
...
mod tests {
    ...
    fn statement_with_subject_sha512(hash_hex: &str) -> String {
        format!(
            r#"{{"_type":"https://in-toto.io/Statement/v1","subject":[{{"name":"artifact","digest":{{"sha512":"{}"}}}}],"predicateType":"https://example.com/predicate/v1","predicate":{{}}}}"#,
            hash_hex
        )
    }
    ...
    #[test]
    fn test_dsse_binding_matching_sha512_subject_from_bytes_ok() {
        let artifact_bytes = b"hello world";
        let hash_hex = hex::encode(sigstore_crypto::sha512(artifact_bytes));
        let envelope = in_toto_envelope(&statement_with_subject_sha512(&hash_hex));
