...
use sigstore_crypto::{CertificateInfo, SigningScheme};
use sigstore_trust_root::TrustedRoot;
use sigstore_types::bundle::VerificationMaterialContent;
use sigstore_types::{
    Artifact, Bundle, DerCertificate, DerPublicKey, HashAlgorithm, SignatureBytes,
    SignatureContent, Statement,
};
use webpki::{anchor_from_trusted_cert, EndEntityCert, KeyUsage, ALL_VERIFICATION_ALGS};

pub(crate) fn compute_artifact_digest_algo(
    artifact: &Artifact<'_>,
    algo: HashAlgorithm,
) -> Result<Vec<u8>> {
    match artifact {
...
pub(crate) fn verify_dsse_artifact_binding(
    ...
    let artifact_sha256 = compute_artifact_digest_algo(artifact, HashAlgorithm::Sha2256)?;
    let artifact_sha256_hex = sigstore_types::Sha256Hash::try_from_slice(&artifact_sha256)
        .map_err(|_| Error::Verification("invalid SHA-256 hash length".to_string()))?
        .to_hex();
    // A digest-only Artifact carries a SHA-256 digest by API contract. SHA-512
    // is therefore computed only when the caller supplied the actual bytes.
    let artifact_sha512_hex = match artifact {
        Artifact::Bytes(bytes) => Some(hex::encode(sigstore_crypto::sha512(bytes))),
    ...
    let subject_matches = statement.matches_sha256(&artifact_sha256_hex)
        || artifact_sha512_hex.as_ref().is_some_and(|sha512| {
            statement
                .subject
                .iter()
                .any(|subject| subject.digest.sha512.as_ref() == Some(sha512))
        });
    if !subject_matches {
        return Err(Error::Verification(
            "artifact SHA-256/SHA-512 does not match any subject in attestation".to_string(),
        ));
    }
...
/// Check if bundle contains V2 tlog entries.
...
/// Extract integrated time from compatible tlog entries with inclusion promises.
///
...
/// 2. The entry is a classic V1 type or npm's legacy intoto/0.0.2 shape
/// 3. The integrated_time is > 0
///
/// The SET is cryptographically verified later by `verify_tlog_entries`; this
/// function only selects a candidate time. No successful result is returned
/// until the SET, inclusion proof/checkpoint, and certificate time all verify.
///
/// Returns the earliest valid integrated time if any are present.
fn extract_integrated_time_with_promise(bundle: &Bundle) -> Option<i64> {
    let mut earliest_time: Option<i64> = None;

    for entry in &bundle.verification_material.tlog_entries {
        let is_classic_v1 = entry.kind_version.version == "0.0.1"
            && (entry.kind_version.kind == "hashedrekord" || entry.kind_version.kind == "dsse");
        let is_legacy_intoto =
            entry.kind_version.version == "0.0.2" && entry.kind_version.kind == "intoto";

        if !(is_classic_v1 || is_legacy_intoto) || entry.inclusion_promise.is_none() {
            continue;
        }
...
/// 2. Integrated time from compatible tlog entries with inclusion promises
///
...
pub fn determine_validation_time(
    ...
    // Integrated time only counts if accompanied by an inclusion promise.
    if let Some(integrated_time) = extract_integrated_time_with_promise(bundle) {
