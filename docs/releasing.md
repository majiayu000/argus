...
管理员对 repository settings、tag、release 和可选 `v1` promotion 的独立授权。
...
重跑 candidate 后，核对每个 target 都产生 `argus-v{version}-{target}` raw binary
与同前缀 archive，且 `release_manifest.json`、`SHA256SUMS`、`LICENSE`、`README.md`
闭集通过 `python3 scripts/verify_release_assets.py --asset-dir release-assets`。生成的
`attestation-plan.json` 只是确定性计划，不是已签名证明。
...
5. 发布负责人单独决定本次版本是否进入 `v1` 渠道。未达到产品 `v1.0` 稳定边界时，
   保持 `v1` 不变；需要晋升时，workflow 只输出 old/new SHA 与 ancestor proof，受限
...
asset 必须停止并调查，不能覆盖。确定性资产必须 byte-for-byte 一致；已有 bundle 只有
在 repo/workflow/tag/commit/subject 全部验证通过后才原样复用，缺失 bundle 才上传本轮
结果。publish 后还必须通过 `gh release verify` 与每个 `verify-asset`。publish 失败
不得移动 `v1`。

## 4. Action 使用与 pin

Release 真正完成前，根 Action 仅是未发布实现。发布后使用者应 pin immutable SemVer
tag 或完整 commit；只有项目明确启用稳定 major 渠道后才使用受保护 major branch。
输入闭集是 `scanType`、
...
security advisory，再发布通过完整流程的新 patch；仅当 `v1` 渠道已经启用时，新的人工
授权之后才能 fast-forward `v1`。通知所有 immutable version/commit pin 用户受影响范围
与替代版本。
