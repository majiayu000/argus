...
comment boundary。普通文件重定向的静态 operand 若为 `/dev/stdin`、
`/dev/stdout`、`/dev/stderr`、`/dev/fd/N`、`/proc/self/fd/N` 或
`/proc/thread-self/fd/N`，必须继承该标准 fd 当前 route；真实文件目标才
改为 `Other`。`&>`/`&>>` 按 Bash 的 `>word 2>&1` 顺序复用同一 operand
parser；含未执行展开的动态 operand 标记为 `Unknown` 并保守保留潜在 pipe
edge，不得静默降级为文件目标。operand 与 wrapper token 共用同一 Bash
quote/backslash tokenizer。分类在任意连续 edge 区间寻找 network→shell；direct
